...
class EdgeConsistencyMLP(nn.Module):
    def __init__(
        ...
        if use_pos_encoding:
            self.pos_encoder = nn.Sequential(
                nn.Linear(3, concat_dim), nn.ReLU(), nn.Linear(concat_dim, concat_dim)
            )
        if use_bn:
    ...
    def forward(
        ...
        features_A,
        features_B,
        patch_centers_A=None,
        patch_centers_B=None,
        batch_offsets=None,
    ):
        x = torch.cat([features_A, features_B], dim=-1)
        if (
            self.use_pos_encoding
            and patch_centers_A is not None
            and patch_centers_B is not None
        ):
