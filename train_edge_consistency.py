...
from dataset.edge_consistency_dataset import (
    EdgeConsistencyDataset,
    edge_consistency_collate,
)
...
def run_epoch(
    ...
    with context:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            pbar.set_postfix(
                {"loss": f"{loss.item():.3f}", "acc": f"{100*accuracy:.1f}%"}
            )
...
def create_data_loaders(config, include_test=False):
    ...
    use_inv_features = config.get("use_inverse_features", False)
    for split in splits:
