...
class OrientedNormalLoss(nn.Module):
    ...
    def forward(self, pred_normals, gt_normals, offset=None):
        ...
        dot_product = (pred_normals * gt_normals_normalized).sum(dim=1)
        pointwise_loss = 1.0 - dot_product
        if offset is not None:
            start_idx = 0
            total_loss = 0.0
...
class SinSimilarityLoss(nn.Module):
    ...
    def forward(self, pred_normals, gt_normals, offset=None):
        losses = {}
        gt_normals_normalized = nn.functional.normalize(gt_normals, p=2, dim=1)
        cross_product = torch.cross(pred_normals, gt_normals_normalized, dim=1)
        sin_similarity = torch.norm(cross_product, dim=1)
...
class EikonalLoss(nn.Module):
    ...
    def forward(self, pred_normals, gt_normals, offset=None):
        norms = torch.norm(pred_normals, dim=1)
...
class UnorientedNormalLoss(nn.Module):
    ...
    def forward(self, pred_normals, gt_normals, offset=None):
        ...
        gt_normals_normalized = nn.functional.normalize(gt_normals, p=2, dim=1)
        dot_product = (pred_normals * gt_normals_normalized).sum(dim=1)
        cos_loss = 1.0 - torch.abs(dot_product)
        ...
        pointwise_loss = cos_loss + inline_loss + eikonal_loss
        total_loss = pointwise_loss.mean()
...
class CombinedNormalLoss(nn.Module):
    ...
    def forward(self, pred_normals, gt_normals, offset=None):
        oriented_loss, oriented_dict = self.oriented_loss(
            pred_normals, gt_normals, offset
        )
        unoriented_loss, unoriented_dict = self.unoriented_loss(
            pred_normals, gt_normals, offset
        )
        total_loss = (
            self.oriented_weight * oriented_loss
...
class BaseWeightedLoss(nn.Module):
    def forward(self, masks, iou_pred, targets, offset=None):
        ...
        for end_idx in offset:
            weight = float(end_idx - start_idx) / total_num
            loss = self.compute_sample_loss(
                mask[start_idx:end_idx], targets[start_idx:end_idx]
            )
            total_loss += weight * loss
            start_idx = end_idx
...
def create_loss_function(config):
    ...
    if loss_type == "focal":
        ...
        return BiFocalLoss(
            focal_alpha=config["training"]["loss"].get("alpha", 0.25),
