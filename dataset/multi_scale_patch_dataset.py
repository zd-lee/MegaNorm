...
class MultiScalePatchDataset(Dataset):
    def __init__(
        ...
        self.model_files = sorted(
            [f for f in os.listdir(data_root) if f.endswith(".ply")]
        )
    ...
    def _compute_scales_hash(self) -> str:
        canonical = []
        canonical.append({"patch_split_version": 2})
        ...
        try:
            ...
            for scale_idx in range(self.num_scales):
                ...
                if len(offsets) > 0 and offsets[-1] != len(indices):
                    return False
                max_points = self.scales[scale_idx]["max_points_per_patch"]
                if len(offsets) > 1 and np.diff(offsets).max() > max_points:
                    return False
            return True
    ...
    def _save_model_cache(
        self, model_idx: int, patches_by_scale: Dict[int, List[torch.Tensor]]
    ):
    ...
    def _build_patch_index(self):
        ...
        if len(invalid_models) == len(self.model_files):
            ...
            self._build_and_cache_all(
                [self.model_files[i] for i in invalid_models],
            ...
            self._load_from_cache(valid_models)
        print(
            f"Dataset ready: {len(self.patch_metadata)} total patches across {self.num_scales} scales"
        )

    def _build_and_cache_all(
        self, model_files: List[str], model_indices: Optional[List[int]] = None
    ):
        ...
        for model_idx, model_file in pbar:
            ...
            for scale_idx, scale_config in enumerate(self.scales):
                ...
                self.model_cache[model_idx][scale_idx] = (
                    coords,
                    gt_normals,
                    patch_list,
                )
    ...
    def _extract_patches_at_scale(
        self, coords: torch.Tensor, scale_config: Dict
    ) -> List[torch.Tensor]:
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        ...
        if self.transform is not None:
            try:
                ...
                print(
                    f"patch_normals shape: {patch_normals.shape if patch_normals is not None else None}"
                )
                raise
